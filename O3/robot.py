...
def plan2(variables):
    """Other side of plan."""
    # universal driving
    if variables["phase"] == "moving_forw_arc":
        variables = moving_forw_arc(variables)
        ...
        variables = drive_straight(variables)
        ...
        variables = small_scan(variables)

    elif variables["phase"] == "smalldecide":
        variables = small_decide(variables)
...
def zerotoobj2(variables):
    """Subpart of zeroing on obj."""
    ...
    # if has detected that it's off object again, save edge degrees and activate next subphase. Calculate goal
        ...
        print("Width of the object:", variables["obj_distance"] * sqrt(
            2 * (1 - cos(radians(abs(variables["l_edge"] - variables["r_edge"]) - 30)))))
...
def zero_to_obj(variables):
    """Check the object and give new values to it."""
    ...
    # if moving counterclockwise to detect edge
        ...
        variables = zerotoobj2(variables)
...
def decideobj2(variables):
    """Decide the correct object."""
...
def decide_objects(variables):
    """Calculate and decide the object."""
    angle_between_second_and_first = abs(variables["first_obj_deg"] - variables["second_obj_deg"])
    first_obj = [variables["first_obj_distance"], variables["first_obj_deg"]]
    second_obj = [variables["second_obj_distance"], variables["second_obj_deg"]]
    if variables["obj_count"] == 2:
        ...
        variables = decideobj2(variables)
    return variables


def drive(variables):
    """Drive forwa rd with given value."""
...
def moving_forw_arc(variables):
    """Move in an arc according to the multiplier."""
    if variables["init"]:
        variables["init"] = False
...
def pre_deg(variables):
    """If the object is on the left side or right, for calculations."""
    goal = -82
...
def drive_straight(variables):
    """Drive straight to get to 20cm from first two objects."""
    # initialisation
    if variables["init1"]:
        variables["init1"] = False
...
def smallscan2(variables):
    """Second part of the small scan."""
    diff = variables["last_fmir"] - variables["fmir"]
    if variables["flag"]:
        variables["counter"] += 1
        if variables["obj_distance"] + 0.10 > variables["fmir"] or variables["side"]:
        ...
        if variables["abs_rota"] - 6 > variables["r_absolute"]:
            ...
            variables["obj_distance"] = variables["past_fmirs"][-4]
            for i in range(6):
                if variables["past_fmirs"][i] > variables["obj_distance"] + 0.1 or variables["past_fmirs"][i] < \
                        variables["obj_distance"] - 0.1:
                    variables["flag"] = False
                    variables["side"] = False
    return variables


def small_scan(variables):
    """Do 180 degree scan to get better object values."""
    ...
    if variables["initsscan"]:
        ...
        # if diff is more than 20cm, then it most likely has detected an object
            ...
            variables = smallscan2(variables)
...
def small_decide(variables):
    ...
    if variables["obj_count"] == 3:
        ...
        variables["yolotravel"] = min(variables["first_obj_distance"], variables["second_obj_distance"]) + 0.10
...
def main():
    ...
    variables["past_fmirs"] = [variables["fmir"], variables["fmir"], variables["fmir"], variables["fmir"],
                               variables["fmir"], variables["fmir"], variables["fmir"], variables["fmir"],
                               variables["fmir"], variables["fmir"]]
