...
def plan(variables):
    ...
    # moves straight until wall or distance and starts scanning again
        ...
        variables = roam_straight(variables)
    ...
    # zeroing to object at long-ish distance (like up to 80cm... Hopefully)
        ...
        variables = zero_to_object(variables)
...
def plan2(variables):
    ...
    # if following line
        ...
        variables = line_follow(variables)
...
def scanning(variables):
    ...
    # if it's not initialisation
        ...
        # if scanning has not reached the limit
            ...
            variables = scanning_non_limit(variables)
    return variables


def scanning_non_limit(variables):
    """Scan for a object."""
...
def zero_to_object(variables):
    ...
    variables = zero_to_object_counter(variables)
    return variables


def zero_to_object_counter(variables):
    """Check if it is a object and turn to the centre of it."""
...
def move_to_obj(variables):
    ...
    # if it's not initialisation
        ...
        if variables["l3"] < 500 or variables["l2"] < 500 or variables["l1"] < 500 or variables["r1"] < 500 or \
                variables["r2"] < 500 or variables["r3"] < 500:
            variables["line"] = True
...
def blind_to_obj(variables):
    """Go blindly to object."""
    # initialisation
    if variables["init1"]:
        variables["init1"] = False
...
def sweep(variables):
    """Initialize sweeping the area for a line and sweep it."""
    # initialisation
    if variables["init1"]:
        variables["init1"] = False
        variables["flag"] = False
    ...
    # if not initialisation
        ...
        variables = sweep_two(variables)
    return variables


def sweep_two(variables):
    """Sweep the area for line after it has started moving."""
    # if has detected a line, stop bot and start line following
    if variables["l3"] < 500 or variables["l2"] < 500 or variables["l1"] < 500 or variables["r1"] < 500 or \
            variables["r2"] < 500 or variables["r3"] < 500:
        variables["left_speed"], variables["right_speed"] = 0, 0
...
def line_follow(variables):
    """Line follower to determine where to put the object."""
    # initialisation
    if variables["init1"]:
        variables["init1"] = False
        variables["flag"] = False
    ...
    # if not initialisation
        ...
        if variables["l3"] < 500 and variables["r3"] < 500:
            ...
            variables["left_speed"], variables["right_speed"] = 0, 0
            variables = line_follow_two(variables)
